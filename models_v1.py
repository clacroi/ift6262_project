...
def model_v10():
    ...
    autoencoder.add(
        Convolution2D(32, 4, 4, border_mode='same', input_shape=(32, 32, 32), dim_ordering='th'))
    autoencoder.add(BatchNormalization(mode=0, axis=1))
    autoencoder.add(Activation('relu'))
    autoencoder.add(MaxPooling2D((2, 2), border_mode='same', dim_ordering='th'))
    ...
    autoencoder.add(UpSampling2D((2, 2), dim_ordering='th'))
    autoencoder.add(
        Convolution2D(3, 5, 5, activation='sigmoid', border_mode='same', input_shape=(32, 32, 32), dim_ordering='th'))
    # Output : (3, 32, 32)
...
def model_v12():
    ...
    encoder.add(
        Convolution2D(64, 5, 5, border_mode='valid', input_shape=(64, 28, 28), dim_ordering='th'))
    encoder.add(BatchNormalization(mode=0, axis=1))
    encoder.add(Activation('relu'))
...
def model_v13():
    ...
    decoder.add(UpSampling2D((2, 2), dim_ordering='th'))
    decoder.add(
        Convolution2D(128, 3, 3, activation='tanh', border_mode='same', input_shape=(256, 4, 4), dim_ordering='th'))

    decoder.add(UpSampling2D((2, 2), dim_ordering='th'))
    decoder.add(
        Convolution2D(64, 3, 3, activation='tanh', border_mode='same', input_shape=(128, 8, 8), dim_ordering='th'))

    decoder.add(UpSampling2D((2, 2), dim_ordering='th'))
    decoder.add(
        Convolution2D(32, 3, 3, activation='tanh', border_mode='same', input_shape=(64, 16, 16), dim_ordering='th'))
